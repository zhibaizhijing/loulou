...
export default async function handler(event: { bookingId: string }, _ctx: unknown): Promise<FnResult<{ declined: boolean }>> {
  try {
    ...
    await (db.collection('bookings').doc(bookingId).update({ data: { status: 'declined', updatedAt: Date.now() } }) as Promise<any>)
