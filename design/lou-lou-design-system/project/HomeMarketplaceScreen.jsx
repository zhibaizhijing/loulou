...
function HomeMarketplaceScreen({ onSearch, onPickService, onPickField }) {
  ...
  const [petType, setPetType] = React.useState({ label: '猫' });
  const [svcType, setSvcType] = React.useState('上门喂养'); // 默认 上门喂养
  const [address, setAddress] = React.useState({ label: '朝阳·三里屯' });
  ...
  const services = [
  { id: '寄养', icon: 'house', hint: '24小时照护' },
  { id: '日托', icon: 'sun', hint: '白天看护，当天接送' },
  { id: '遛狗', icon: 'sneaker', hint: '至少30分钟' },
  { id: '上门喂养', icon: 'hand-waving', hint: '查看、喂食、换水、铲屎等至少30分钟' },
  { id: '伴宠留宿', icon: 'moon-stars', hint: '守护者上门陪伴/过夜' }];
  ...
  // 常用守护者（最近预约过的） — horizontal row of circular avatars
  const recents = [
  { id: 'r1', name: '林若', bg: LL.butter },
  { id: 'r2', name: '阿浩', bg: LL.peach },
  { id: 'r3', name: '桃子', bg: LL.mint },
  { id: 'r4', name: '陈逸', bg: LL.lavender },
  { id: 'r5', name: '小米', bg: '#E8E3F2' },
  { id: 'r6', name: '阿哲', bg: LL.butter },
  { id: 'r7', name: '若曦', bg: LL.peach }];
...
// ─── Brand header — fixed-feel hero with logo image + slogan ───
  ...
  return (
    <div style={{
      ...
      <img
        ...
        onError={(e) => {
          // Fallback wordmark if the logo image fails to load
          const span = document.createElement('span');
          span.innerText = 'Loulou 噜噜';
          span.style.cssText =
          'font-family: "Brush Script MT", "Snell Roundhand", "STKaiti", cursive;' +
          'font-size: 30px; font-weight: 700; color:#1E1E24; letter-spacing:0.02em;';
          e.target.replaceWith(span);
        }} />
      
      <div style={{
        fontSize: 13, color: LL.text2, fontWeight: 500, letterSpacing: '0.04em'
      }}>让每一次分离都安心</div>
    </div>);
...
function ServiceIconTile({ id, icon, selected, onClick }) {
  ...
  return (
    <button
      ...
      <div style={{ ...{
          ...
          background: selected ? LL.ink : 'rgba(34,40,44,0.06)',
          color: selected ? '#fff' : LL.text,
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          transition: 'background 160ms ease, color 160ms ease'
        }, background: "rgb(254, 231, 166)" }}>
        <i className={`${selected ? 'ph-fill' : 'ph'} ph-${icon}`} style={{ fontSize: 22, color: "rgb(255, 255, 255)" }} />
      </div>
      <div style={{
  ...
  return (
    <div style={{ marginTop: 14 }}>
      ...
      <div style={{
        ...
        WebkitOverflowScrolling: 'touch', gap: "20px"
      }}>
        {items.map((g) =>
        <button
          key={g.id}
          ...
          style={{
            ...
            <div style={{
            width: 52, height: 52, borderRadius: '50%', background: g.bg,
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            ...
            <div style={{
            fontSize: 11, color: LL.text, fontWeight: 500,
            maxWidth: 56, whiteSpace: 'nowrap', overflow: 'hidden',
