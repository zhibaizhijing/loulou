...
function SearchResultsScreen({ onBack, query, onPickField, setTopBarLeading }) {
  ...
  const guardians = [
  { id: 'g1', name: '林若 Lily', photo: photo('1568822617270-2c1579f8dfe2', 'guardianPhoto1'), bio: '养狗6年，专业寄养经验3年，自家带院子', rating: 4.97, reviews: 128, dist: 0.8, city: '三里屯', features: ['认证5年', '户外活动', '清洁消毒'], price: 168, unit: '晚', orders: 412, cert: true, reused: true, badge: '金牌' },
  ...
  { id: 'g4', name: '陈逸 Yi', photo: photo('1542909192-2f2241a99c9d', 'guardianPhoto4'), bio: '住家守护8年，接待中大型犬，提供日常视频', rating: 4.86, reviews: 212, dist: 2.3, city: '国贸', features: ['认证8年', '24h智能监控', '户外活动', '清洁消毒'], price: 228, unit: '晚', orders: 534, cert: true, reused: false, badge: '金牌' },
  ...
  return (
    <div style={{ position: 'relative', background: LL.bg, minHeight: '100%' }}>
      ...
      <div style={{
        ...
        boxShadow: '0 1px 0 rgba(0,0,0,0.06), 0 4px 12px rgba(0,0,0,0.04)', padding: "0px"
      }}>
        {/* Brand row — back left · logo+slogan centre · spacer right */}
        <div style={{
          display: 'flex', alignItems: 'flex-start', padding: '16px 12px 4px', gap: 4
        }}>
          {/* Back button */}
          ...
          <div style={{
            ...
            <img
              src={typeof window !== 'undefined' && window.__resources && window.__resources.loulouLogo || './assets/loulou-logo.png'}
              alt="Loulou 噜噜"
              style={{ height: 30, width: 'auto', display: 'block' }}
              onError={(e) => {
                const span = document.createElement('span');
                span.innerText = 'Loulou 噜噜';
                span.style.cssText = 'font-family:"Brush Script MT",cursive;font-size:22px;font-weight:700;color:#1E1E24;';
                e.target.replaceWith(span);
              }} />
            
            <div style={{ fontSize: 10.5, color: LL.text3, fontWeight: 500, letterSpacing: '0.04em' }}>
              让每一次分离都安心
            </div>
          </div>
          {/* Spacer balances back button to keep logo optically centred */}
          <div style={{ width: 32, flex: '0 0 auto' }} />
        </div>

        {/* Compact 2-row search summary, full-width */}
        <div style={{ display: 'flex', alignItems: 'center', padding: '0 14px 6px' }}>
          <CompactSearchSummary q={q} onPickField={onPickField} />
        </div>

        {/* Filter bar — single 筛选 button + sort dropdown */}
        <div style={{
      ...
      <div style={{
        ...
        <div style={{ fontSize: 13.5, color: LL.text2 }}>
          共 <span style={{ color: LL.text, fontWeight: 700, fontSize: 15, fontVariantNumeric: 'tabular-nums' }}>{visible.length}</span> 位守护者
        </div>
      ...
      <div style={{ padding: '0 12px 28px', display: 'flex', flexDirection: 'column', gap: 10 }}>
        ...
        <div style={{
          ...
          textAlign: 'center', boxShadow: '0 2px 8px rgba(0,0,0,0.04)'
            ...
            <div style={{ fontSize: 14, fontWeight: 700, color: LL.text }}>暂无匹配的守护者</div>
...
function CompactSearchSummary({ q, onPickField }) {
  return (
    <div style={{
      ...
      {/* Row 1: pet · service · address (3 fields fill full width equally) */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
        <SummaryChip icon="paw-print" value={q.petType} onClick={() => onPickField?.('petType')} flex />
        <SummaryDot />
        <SummaryChip icon="hand-heart" value={q.svcType} onClick={() => onPickField?.('svcType')} flex />
        <SummaryDot />
        <SummaryChip icon="map-pin" value={q.address} onClick={() => onPickField?.('address')} flex />
      </div>
      {/* Row 2: date range (Form A) or schedule + period (Form B) */}
      <div style={{
        ...
        <>
            ...
            <i className="ph ph-arrow-right" style={{ fontSize: 12, color: LL.text3, padding: '0 2px', flex: '0 0 auto' }} />
            <SummaryChip value={q.endDate} onClick={() => onPickField?.('endDate')} flex />
            {q.nights > 0 &&
          <div style={{
            marginLeft: 2, fontSize: 11, color: LL.text2,
...
// ─────────────────────────────────────────────────────────────
function FilterButton({ count = 0, onClick }) {
  ...
  return (
    <button onClick={(e) => {e.stopPropagation();onClick();}} style={{
      ...
      fontSize: 13, fontWeight: active ? 700 : 600, fontFamily: LL.font,
      ...
      {count > 0 &&
      <span style={{
        background: '#fff', color: LL.ink, borderRadius: 999,
        padding: '0 6px', minWidth: 16, height: 16,
