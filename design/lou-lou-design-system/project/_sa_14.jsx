...
// ─── Pet tab icon — handles hamster with inline SVG ─────────
function PetTabIcon({ type, size = 12 }) {
  if (type === 'hamster') {
    return (
      <svg width={size} height={size} viewBox="0 0 24 24" fill="none"
        ...
        <path d="M18 16 Q21 13 19.5 10"/>
      </svg>
    );
  }
  const n = PET_ICON_MAP2[type];
  if (!n) return null;
  return <i className={`ph ph-${n}`} style={{ fontSize:size, lineHeight:1 }} />;
}
        ...
        {tabs.map(t => {
          ...
          return (
            <button key={t.type} onClick={() => setActiveType(t.type)} style={{
              ...
              <PetTabIcon type={t.type} size={12} />
              {t.label}
            </button>
          );
        })}
