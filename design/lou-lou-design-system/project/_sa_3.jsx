  ...
  return (
    <>
      <div onClick={onClose} style={{
        position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.35)', zIndex: 85,
      }} />
      <div style={{
      ...
      <div style={{ padding: '8px 14px 12px' }}>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          <ScheduleTypeBtn
            ...
            iconName="calendar-blank" label="单次预约"
            sub="选择单天或连续多天"
          />
          <ScheduleTypeBtn
            on={draft.type === 'recurring'}
            onClick={() => setDraft({ ...draft, type: 'recurring' })}
            iconName="arrows-clockwise" label="每周重复"
            sub="选择固定周几"
          />
        </div>
        ...
        {draft.type === 'once' ? (
          <>
            ...
            {showPickMode && pickMode === 'single' ? (
              <>
                <RangeCalendar mode="multi" selectedDays={pickedDays} onToggleDay={toggleDay} />
...
function ScheduleTypeBtn({ on, onClick, iconName, label, sub }) {
  return (
    <button onClick={onClick} style={{
      ...
      <i className={`ph ph-${iconName}`} style={{ fontSize: 22, color: LL.text, lineHeight: 1 }} />
