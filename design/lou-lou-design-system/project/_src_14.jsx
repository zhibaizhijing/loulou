...
function GuardianCalendar({
  ...
  const isPast   = d => toDate(d) < TODAY;
  const isTodayD = d => month === TODAY.getMonth() && d === TODAY.getDate();
  const isBooked = d => bookedSet.has(toKey(d));
  const sameDate = (a, b) => a && b && a.getFullYear()===b.getFullYear()
                              && a.getMonth()===b.getMonth() && a.getDate()===b.getDate();
  const isStart  = d => !viewOnly && sameDate(toDate(d), start);
  const isEnd    = d => !viewOnly && sameDate(toDate(d), end);
  const isInRange= d => !viewOnly && start && end && toDate(d) > start && toDate(d) < end;

  const handleTap = d => {
    if (viewOnly || isPast(d) || isBooked(d)) return;
    const dt = toDate(d);
  ...
  return (
    <div>
      ...
      {/* Day cells — gap:0 so range band is continuous */}
      <div style={{ display:'grid', gridTemplateColumns:'repeat(7,1fr)', gap:0 }}>
        {cells.map((d, i) => {
          if (!d) return <div key={`e${i}`} style={{ height: LABEL_H+BOX_H+PRICE_H+2 }} />;

          const past     = isPast(d);
          const today    = isTodayD(d);
          const booked   = isBooked(d);
          const avail    = !past && !booked;
          const isStartD = isStart(d);
          const isEndD   = isEnd(d);
          const isInRangeD = isInRange(d);
