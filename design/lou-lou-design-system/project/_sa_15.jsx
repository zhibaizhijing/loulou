...
function FInput({ value, onChange, placeholder, type = 'text', max, inputMode }) {
  return (
...
function AddEditPetForm({ initialPet, onSave, onDiscard, onAutoSave, saveLabel }) {
  ...
  const stepBasic = (
    <div style={{ background:'#fff', padding:'18px 16px 4px' }}>
      ...
      <FField label="出生日期">
        <FInput type="date" value={pet.dob} onChange={v => set('dob', v)} placeholder="选择出生日期" max={new Date().toISOString().slice(0,10)} />
        {age && <div style={{ fontSize:12.5, color:LL.text3, marginTop:7 }}>年龄：{age}</div>}
      </FField>

      <FField label="体重（公斤）">
        <FInput type="text" inputMode="decimal" value={pet.weight} onChange={v => set('weight', v)} placeholder="例：5.5" />
      </FField>
